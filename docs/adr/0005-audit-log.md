...
Actions use `resource.action`, for example `membership.role_changed`, `membership.disabled`, `student.created`, and `student.updated`. Entity types are uppercase (`MEMBERSHIP`, `STUDENT`). LOCAL-02 covers control-plane membership/invitation actions and tenant Student create/update only. Course, Class, Teacher, Enrollment, Schedule, Attendance, Billing, CRM, and other domain mutations remain deferred; each future module must call the audit writer explicitly inside its owning transaction and provide trusted actor context, allowlisted business fields, and endpoint/isolation tests before adoption.
