...
export const openApiSchemas = {
  ...
  CommunicationEventType: {
    type: 'string',
    enum: ['SESSION_REMINDER', 'SCHEDULE_CHANGED', 'ATTENDANCE_ABSENCE', 'TUITION_DUE', 'TUITION_OVERDUE', 'PAYMENT_RECEIVED', 'TRIAL_REMINDER', 'ENROLLMENT_EXPIRING'],
  },
  CommunicationChannel: { type: 'string', enum: ['IN_APP', 'EMAIL'] },
  CommunicationStatus: { type: 'string', enum: ['PENDING', 'SENT', 'FAILED', 'CANCELLED'] },
  CommunicationMessage: {
    type: 'object',
    description: 'Rendered communication snapshot. subject/body are frozen at creation; template edits never change history, and retries reuse the same snapshot and destination. provider LOCAL means accepted by the local development adapter, not a real mailbox.',
    properties: {
      id: ulid,
      eventType: { $ref: '#/components/schemas/CommunicationEventType' },
      channel: { $ref: '#/components/schemas/CommunicationChannel' },
      recipientType: { type: 'string', enum: ['STUDENT', 'GUARDIAN', 'LEAD', 'MEMBERSHIP'] },
      recipientId: { ...ulid, nullable: true },
      recipientName: nullableString,
      destination: { ...nullableString, description: 'Normalized email snapshot; EMAIL messages only' },
      subject: nullableString,
      body: { type: 'string' },
      relatedEntityType: nullableString,
      relatedEntityId: { ...ulid, nullable: true },
      dedupeKey: { type: 'string', description: 'Deterministic logical-event identity; unique per tenant' },
      status: { $ref: '#/components/schemas/CommunicationStatus' },
      provider: nullableString,
      providerMessageId: nullableString,
      attemptCount: { type: 'integer' },
      lastError: { ...nullableString, description: 'Sanitized delivery error; never contains secrets or PII' },
      createdAt: { type: 'string', format: 'date-time' },
      updatedAt: { type: 'string', format: 'date-time' },
      sentAt: { ...nullableString, format: 'date-time' },
      failedAt: { ...nullableString, format: 'date-time' },
    },
    required: ['id', 'eventType', 'channel', 'recipientType', 'recipientId', 'recipientName', 'destination', 'subject', 'body', 'relatedEntityType', 'relatedEntityId', 'dedupeKey', 'status', 'provider', 'providerMessageId', 'attemptCount', 'lastError', 'createdAt', 'updatedAt', 'sentAt', 'failedAt'],
  },
  CommunicationHistory: {
    type: 'object',
    description: 'Cursor-paginated communication history. Filters (eventType, channel, status, from, to, recipient, studentId, leadId) are all tenant-scoped.',
    properties: {
      data: { type: 'array', items: { $ref: '#/components/schemas/CommunicationMessage' } },
  ...
  CommunicationTemplate: {
    type: 'object',
    description: 'Effective template view per event and channel: tenant override when enabled, otherwise the built-in default. disabled overrides suppress dispatch for that event/channel.',
    properties: {
      eventType: { $ref: '#/components/schemas/CommunicationEventType' },
      channel: { $ref: '#/components/schemas/CommunicationChannel' },
      timing: { type: 'string', enum: ['IMMEDIATE', 'TIME_BASED'] },
      variables: { type: 'array', items: { type: 'object', properties: { name: { type: 'string' }, required: { type: 'boolean' } }, required: ['name', 'required'] } },
      hasOverride: { type: 'boolean' },
      enabled: { type: 'boolean' },
      effectiveSubject: nullableString,
      effectiveBody: { type: 'string' },
    },
    required: ['eventType', 'channel', 'timing', 'variables', 'hasOverride', 'enabled', 'effectiveSubject', 'effectiveBody'],
  },
  CommunicationTemplatePreview: {
    type: 'object',
    description: 'Rendered preview using safe fixture context; no message is persisted.',
    properties: {
      eventType: { $ref: '#/components/schemas/CommunicationEventType' },
      channel: { $ref: '#/components/schemas/CommunicationChannel' },
      subject: nullableString,
