...
export function AppShell() {
  ...
  return <div className="min-h-screen bg-[#f4f6f8] lg:grid lg:grid-cols-[248px_minmax(0,1fr)]">
    ...
    <Dialog open={navigationOpen} onOpenChange={setNavigationOpen}>
      <DialogContent className="inset-y-0 left-0 top-0 h-dvh w-[min(320px,88vw)] max-w-none translate-x-0 translate-y-0 border-0 p-0 [&>button]:text-white [&>button:hover]:bg-white/10 lg:hidden">
    ...
    <div className="min-w-0 lg:col-start-2">
      <Topbar pathname={location.pathname} centerName={membership?.tenant.name} onOpenNavigation={() => setNavigationOpen(true)} />
      <Outlet />
    </div>
  </div>;
}
