...
const defaults: ClassFormValues = { courseId: "", branchId: "", courseLevelId: "", defaultRoomId: "", primaryTeacherId: "", capacity: "", startDate: "", expectedEndDate: "", code: "", name: "", description: "", status: "ACTIVE" };
...
export function ClassForm() {
  ...
  const { register, handleSubmit, reset, setError, setValue, watch, formState: { errors } } = useForm<ClassFormValues>({ resolver: zodResolver(classSchema), defaultValues: defaults });
  ...
  return <PageContainer className="max-w-5xl">
    ...
    <form className="form-sheet" onSubmit={handleSubmit((values) => mutation.mutate(values))} noValidate>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <FormField id="class-start-date" label="Ngày bắt đầu" error={errors.startDate?.message}><Input id="class-start-date" type="date" {...register("startDate")} /></FormField>
        <FormField id="class-end-date" label="Ngày dự kiến kết thúc" error={errors.expectedEndDate?.message}><Input id="class-end-date" type="date" {...register("expectedEndDate")} /></FormField>
