...
const defaults: ClassFormValues = { code: "", name: "", description: "", status: "ACTIVE" };
...
export function ClassForm() {
  ...
  const mutation = useMutation({
    ...
    onError: (error) => setError("root", { message: axios.isAxiosError(error) && error.response?.status === 409 ? "Mã lớp học đã tồn tại trong trung tâm." : "Không thể lưu lớp học. Vui lòng thử lại." }),
  });
