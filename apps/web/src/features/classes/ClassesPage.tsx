...
const columns = (canWrite: boolean) => [
  ...
  column.display({ id: "dates", header: "Thời gian", cell: ({ row }) => row.original.startDate || row.original.expectedEndDate ? `${row.original.startDate?.slice(0, 10) ?? "?"} – ${row.original.expectedEndDate?.slice(0, 10) ?? "?"}` : "—" }),
