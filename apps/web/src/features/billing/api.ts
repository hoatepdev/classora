...
export async function listInvoices() { return (await api.get<BillingInvoice[]>("/invoices")).data; }
export async function listPayments() { return (await api.get<BillingPayment[]>("/payments")).data; }
export async function listReceivables() { return (await api.get<BillingReceivable[]>("/receivables")).data; }
export async function listPrices() { return (await api.get<BillingPrice[]>("/pricing-plans")).data; }
export async function listDiscounts() { return (await api.get<BillingDiscount[]>("/discounts")).data; }
export async function listRefunds() { return (await api.get<BillingRefund[]>("/refunds")).data; }
export async function recordPayment(input: RecordPaymentInput) { return (await api.post<BillingPayment>(`/invoices/${input.invoiceId}/payments`, input)).data; }
export async function requestRefund(input: RequestRefundInput) { return (await api.post<BillingRefund>(`/payments/${input.paymentId}/refund`, input)).data; }
