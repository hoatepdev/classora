...
const displayMoney = (value?: number | null) => typeof value === "number" ? money.format(value) : "—";
...
function PaymentDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) { const queryClient = useQueryClient(); const [invoiceId, setInvoiceId] = useState(""); const [amount, setAmount] = useState(""); const [method, setMethod] = useState("CASH"); const [paidAt, setPaidAt] = useState(new Date().toISOString().slice(0, 10)); const mutation = useMutation({ mutationFn: () => recordPayment({ invoiceId, amount: Number(amount), method, paidAt }), onSuccess: () => { void queryClient.invalidateQueries({ queryKey: billingPaymentsQueryKey() }); void queryClient.invalidateQueries({ queryKey: billingOverviewQueryKey() }); onOpenChange(false); toast.success("Đã ghi nhận thanh toán."); }, onError: (error) => toast.error(getApiErrorMessage(error, "Không thể ghi nhận thanh toán.")) }); return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle>Ghi nhận thanh toán</DialogTitle><DialogDescription>Nhập thông tin khoản thu đã nhận từ học viên.</DialogDescription></DialogHeader><div className="grid gap-4"><Field label="Mã hóa đơn"><Input value={invoiceId} onChange={(e) => setInvoiceId(e.target.value)} placeholder="ID hóa đơn" /></Field><Field label="Số tiền"><Input type="number" min="0" value={amount} onChange={(e) => setAmount(e.target.value)} /></Field><Field label="Ngày thu"><Input type="date" value={paidAt} onChange={(e) => setPaidAt(e.target.value)} /></Field><Field label="Phương thức"><select className="min-h-10.5 rounded-lg border border-input bg-card px-3 text-sm" value={method} onChange={(e) => setMethod(e.target.value)}><option value="CASH">Tiền mặt</option><option value="BANK_TRANSFER">Chuyển khoản</option><option value="CARD">Thẻ</option></select></Field></div><DialogFooter><Button variant="secondary" onClick={() => onOpenChange(false)}>Hủy</Button><Button disabled={mutation.isPending || !invoiceId || Number(amount) <= 0} onClick={() => mutation.mutate()}>Lưu thanh toán</Button></DialogFooter></DialogContent></Dialog>; }
function RefundDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) { const queryClient = useQueryClient(); const [paymentId, setPaymentId] = useState(""); const [amount, setAmount] = useState(""); const [reason, setReason] = useState(""); const mutation = useMutation({ mutationFn: () => requestRefund({ paymentId, amount: Number(amount), reason }), onSuccess: () => { void queryClient.invalidateQueries({ queryKey: billingRefundsQueryKey() }); onOpenChange(false); toast.success("Đã tạo yêu cầu hoàn tiền."); }, onError: (error) => toast.error(getApiErrorMessage(error, "Không thể tạo yêu cầu hoàn tiền.")) }); return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle>Tạo yêu cầu hoàn tiền</DialogTitle><DialogDescription>Yêu cầu sẽ được lưu để bộ phận có quyền xử lý tiếp.</DialogDescription></DialogHeader><div className="grid gap-4"><Field label="Mã thanh toán"><Input value={paymentId} onChange={(e) => setPaymentId(e.target.value)} placeholder="ID thanh toán" /></Field><Field label="Số tiền"><Input type="number" min="0" value={amount} onChange={(e) => setAmount(e.target.value)} /></Field><Field label="Lý do"><Input value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Nhập lý do hoàn tiền" /></Field></div><DialogFooter><Button variant="secondary" onClick={() => onOpenChange(false)}>Hủy</Button><Button disabled={mutation.isPending || !paymentId || Number(amount) <= 0 || !reason.trim()} onClick={() => mutation.mutate()}>Tạo yêu cầu</Button></DialogFooter></DialogContent></Dialog>; }
